...
oc_notify_pending_execution_msg = "⚠️ Заказ <code>#{order_id}</code> от <code>{username}</code> ожидает вашего внимания уже более {} минут!"
oc_notify_pending_confirmation_msg = "🔔 Покупатель <code>{username}</code> не подтверждает заказ <code>#{order_id}</code> уже более {} часов. Возможно, стоит ему напомнить."
