# START OF FILE FunPayCortex/locales/uk.py
...
gl_refresh_and_try_again = "Оновіть список і спробуйте знову." # Для auto_delivery_cp.py
...
gs_nultidelivery = "{} Мульти-видача" # Примітка: "nultidelivery" виглядає як помилка, припускаємо "Мульти-видача"
...
ar_add_more = "➕ Ще"
ar_add_another = "➕ Іншу"
ar_no_valid_commands_entered = "Не введено жодної коректної команди." # Для auto_response_cp.py
ar_default_response_text = "Відповідь для цієї команди ще не налаштована. Відредагуйте її." # Для auto_response_cp.py
ar_default_notification_text = "Користувач $username ввів команду: $message_text" # Для auto_response_cp.py
ar_command_deleted_successfully = "Команду/сет '{command_name}' успішно видалено." # Для auto_response_cp.py
...
# ad_to_mm вже є вище
never_updated = "ніколи" # Для auto_delivery_cp.py (час останнього оновлення списку лотів FunPay)
ad_default_response_text_new_lot = "Дякуємо за покупку, $username!\nОсь твій товар:\n$product" # Для auto_delivery_cp.py
...
lot_info_header = "Інформація про лот" # Для utils.py -> generate_lot_info_text
text_not_set = "(текст не задано)" # Для utils.py -> generate_lot_info_text
gf_infinity = "нескінченно" # Для utils.py -> generate_lot_info_text
gf_count_error = "помилка підрахунку" # Для utils.py -> generate_lot_info_text
gf_file_not_found_short = "не знайдено" # Для utils.py -> generate_lot_info_text
gf_not_linked = "не прив'язано" # Для utils.py -> generate_lot_info_text
gf_file_created_now = "створено" # Для utils.py -> generate_lot_info_text (Коли файл щойно створений утилітою)
gf_file_creation_error_short = "помилка створення" # Для utils.py -> generate_lot_info_text
no_lots_using_file = "Жоден лот не використовує цей файл." # Для auto_delivery_cp.py
gf_no_products_to_add = "Не було введено товарів для додавання." # Для auto_delivery_cp.py
gf_deleted_successfully = "Файл {file_name} успішно видалено." # Для auto_delivery_cp.py
gf_already_deleted = "Файл {file_name} вже було видалено." # Для auto_delivery_cp.py
...
tmplt_editing_header = "📝 Редагування шаблону" # Для templates_cp.py
tmplt_err_empty_text = "❌ Текст шаблону не може бути порожнім. Будь ласка, введіть текст." # Для templates_cp.py
tmplt_deleted_successfully = "✅ Шаблон «{template_text}» успішно видалено." # Для templates_cp.py
...
pl_status_active = "Статус: Активний 🚀" # Для plugins_cp.py
pl_status_inactive = "Статус: Неактивний 💤" # Для plugins_cp.py
pl_no_commands = "У цього плагіна немає зареєстрованих команд." # Для plugins_cp.py
pl_delete_handler_failed = "⚠️ Помилка при виконанні обробника видалення плагіна '{plugin_name}'. Плагін видалено зі списку, але його дані або файли могли залишитися. Перевірте логи." # Для plugins_cp.py
pl_deleted_successfully = "✅ Плагін '{plugin_name}' успішно видалено. Перезапустіть FPCortex, щоб зміни набули чинності." # Для plugins_cp.py
pl_file_delete_error = "⚠️ Не вдалося видалити файл плагіна '{plugin_path}'. Перевірте права доступу та логи." # Для plugins_cp.py

# Конфіги
cfg_download_main = "📥 Основний конфіг"
cfg_download_ar = "📥 Конфіг автовідповіді"
cfg_download_ad = "📥 Конфіг автовидачі"
cfg_upload_main = "📤 Основний конфіг"
cfg_upload_ar = "📤 Конфіг автовідповіді"
cfg_upload_ad = "📤 Конфіг автовидачі"
...
proxy_status_enabled = "Увімкнено" # Для proxy_cp.py
proxy_status_disabled = "Вимкнено" # Для proxy_cp.py
proxy_check_status_enabled = "Увімкнена" # Для proxy_cp.py
proxy_check_status_disabled = "Вимкнена" # Для proxy_cp.py
proxy_not_used_currently = "не використовується" # Для proxy_cp.py
proxy_not_selected = "не обрано" # Для proxy_cp.py
proxy_check_interval_info = "Інтервал авто-перевірки: {interval} хв." # Для proxy_cp.py
proxy_global_status_header = "Загальний статус проксі-модуля" # Для proxy_cp.py
proxy_module_status_label = "Стан модуля:" # Для proxy_cp.py
proxy_health_check_label = "Авто-перевірка проксі:" # Для proxy_cp.py
proxy_current_in_use_label = "Поточний проксі в роботі:" # Для proxy_cp.py
proxy_select_error_not_found = "⚠️ Помилка: цей проксі більше не існує у списку." # Для proxy_cp.py
proxy_select_error_invalid_format = "⚠️ Помилка: обраний проксі має невірний формат." # Для proxy_cp.py
proxy_selected_and_applied = "✅ Проксі {proxy_str} обрано та застосовано." # Для proxy_cp.py
proxy_selected_not_applied = "ℹ️ Проксі {proxy_str} обрано. Увімкніть модуль проксі в налаштуваннях, щоб він почав використовуватися." # Для proxy_cp.py
proxy_deleted_successfully = "✅ Проксі {proxy_str} успішно видалено зі списку." # Для proxy_cp.py
proxy_delete_error_not_found = "⚠️ Помилка: проксі для видалення не знайдено у списку." # Для proxy_cp.py

# Посилання
# Прибрані згідно з вимогою
...
param_disabled = "❌ Це налаштування вимкнено глобально і не може бути змінено для даного лота.\n\n" \
...
✅ <i>FPCortex</i> точно визначає автора: ти, співрозмовник чи арбітраж.
...
✅ Працює трохи швидше.
❌ <i>FPCortex</i> бачить лише останнє повідомлення. Якщо прийде кілька повідомлень поспіль – побачить лише останнє.
❌ <i>FPCortex</i> не бачить зображення та не може їх переслати.
...
💡 Якщо натиснути кнопку <code>Детальніше</code> у сповіщенні, <i>FPCortex</i> "прочитає" чат, покаже останні 15 повідомлень (включаючи картинки) та зможе точно визначити автора."""
bot_started = """✅ Telegram-бот запущено!
Ти можеш <b><u>налаштовувати конфіги</u></b> та <b><u>використовувати всі функції <i>Telegram</i>-бота</u></b>.

⏳ <i>FPCortex</i> <b><u>поки не ініціалізовано</u></b>, його функції неактивні.
...
<b>⏱️ Інше:</b>
    Аптайм:  <code>{}</code>
    ID чату:  <code>{}</code>"""
act_blacklist = """Введи нікнейм користувача для додавання до чорного списку."""
already_blacklisted = "❌ Користувач <code>{}</code> вже у чорному списку."
user_blacklisted = "✅ Користувача <code>{}</code> додано до чорного списку."
...
user_unbanned = "✅ Користувача <code>{}</code> видалено з чорного списку."
...
proxy_format = "❌ Неправильний формат проксі. Потрібно: <u>login:password@ip:port</u> або <u>ip:port</u>."
...
act_edit_watermark = "Введи новий текст водяного знака. Приклади:\n{}\n<code>𝑭𝒖𝒏𝑷𝒂𝒚 𝑪𝒐𝒓𝒕𝒆𝒙</code>\n" \
                     ...
                     "<code>FPCx</code>\n<code>🤖</code>\n<code>🧠</code>\n\n" \
                     "Приклади можна скопіювати та відредагувати.\nМай на увазі, що на FunPay емодзі " \
...
update_no_tags = "❌ Не вдалося отримати список версій. Спробуй пізніше. (Оновлення вимкнено)"
update_lasted = "✅ У тебе остання версія FPCortex {} (Оновлення вимкнено)"
update_get_error = "❌ Не вдалося отримати інформацію про нову версію. Спробуй пізніше. (Оновлення вимкнено)"
update_available = "<b>✨ Доступна нова версія! ✨</b>\n\n\n{}\n\n{}"
update_update = "Для оновлення введи команду /update (Оновлення вимкнено)"
update_backup = "✅ Створено резервну копію (конфіги, сховище, плагіни): <code>backup.zip</code>.\n\n" \
                "🔒 <b>ВАЖЛИВО:</b> НЕ НАДСИЛАЙ цей архів НІКОМУ. Він містить усі дані та налаштування бота (включаючи golden_key та товари)."
update_backup_error = "⚠️ Не вдалося створити резервну копію."
update_backup_not_found = "❌ Резервну копію не знайдено."
update_downloaded = "✅ Завантажено оновлення {} (пропущено {} елементів). Встановлюю... (Оновлення вимкнено)"
update_download_error = "⚠️ Помилка при завантаженні оновлення."
update_done = "✅ Оновлення встановлено! Перезапусти <i>FPCortex</i> командою /restart (Оновлення вимкнено)"
update_done_exe = "✅ Оновлення встановлено! Новий <code>FPCortex.exe</code> у папці <code>update</code>. " \
                  "Вимкни <i>FPCortex</i>, заміни старий <code>FPCortex.exe</code> новим " \
                  "та запусти <code>Start.bat</code>. (Оновлення вимкнено)"
update_install_error = "⚠️ Помилка при встановленні оновлення."
...
доведеться заходити на сервер/комп'ютер та запускати мене вручну! 💻"""
...
review_reply_empty = "⚠️ Відповідь на {}⭐ відгук не задана."
...
updating_profile = "Оновлюю статистику акаунту... 📊"
profile_updating_error = "⚠️ Не вдалося оновити статистику."
acc_balance_available = "доступно" # Для utils.py -> generate_profile_text
...
ad_gf_unlinked = "✅ Файл товарів відв'язано від лота <code>{}</code>."
ad_gf_linked = "✅ Файл <code>storage/products/{}</code> прив'язано до лота <code>{}</code>."
ad_gf_created_and_linked = "✅ Файл <code>storage/products/{}</code> <b><u>створено</u></b> та прив'язано до лота <code>{}</code>."
ad_creating_gf = "⏳ Створюю файл <code>storage/products/{}</code>..."
ad_product_var_err = "⚠️ До лота <code>{}</code> прив'язано файл товарів, але в тексті видачі немає змінної <code>$product</code>."
...
gf_name_invalid = "🚫 Неприпустиме ім'я файлу.\n\n" \
                  "Дозволені тільки <b><u>англійські</u></b> " \
                  "та <b><u>українські</u></b> літери, цифри, а також символи <code>_</code>, <code>-</code> та <code>пробіл</code>."
...
gf_add_goods_err = "⚠️ Не вдалося додати товари до файлу."
gf_new_goods = "✅ Додано <code>{}</code> товар(-ів) у файл <code>storage/products/{}</code>."
gf_empty_error = "📂 Файл storage/products/{} порожній."
gf_linked_err = "⚠️ Файл <code>storage/products/{}</code> прив'язаний до одного або декількох лотів.\n" \
...
ar_enter_new_cmd = "Введи нову команду (або декілька, розділяючи їх символом <code>|</code>)."
...
adv_fpc = "😎 FPCortex - твій найкращий помічник для FunPay!"
...
# - Описи меню
desc_main = "Привіт! Обери, що налаштуємо 👇"
desc_lang = "Обери мову інтерфейсу:"
...
desc_bl = "Встанови обмеження для користувачів з чорного списку."
desc_ar = "Додай команди для автовідповідача або відредагуй існуючі."
desc_ar_list = "Обери команду або набір команд для редагування:"
desc_ad = "Налаштування автовидачі: прив'язка до лотів, керування файлами товарів тощо."
desc_ad_list = "Список лотів з налаштованою автовидачею. Обери лот для редагування:"
desc_ad_fp_lot_list = "Список лотів з твого профілю FunPay. Обери лот, щоб налаштувати автовидачу.\n" \
                      "Якщо потрібного лота немає, натисни <code>🔄 Оновити</code>.\n\n" \
                      "Останнє сканування: {}"
desc_gf = "Обери файл товарів для керування:"
desc_mv = "Налаштуй, як виглядатимуть сповіщення про нові повідомлення."
desc_gr = "Налаштуй вітальні повідомлення для нових діалогів.\n\n<b>Поточний текст привітання:</b>\n<code>{}</code>"
desc_oc = "Налаштуй повідомлення, яке надсилатиметься при підтвердженні замовлення.\n\n<b>Поточний текст повідомлення:</b>\n<code>{}</code>"
...
# profile_header_title = "Статистика акаунта" # Вже є як cmd_profile, але можна зробити окремий
cmd_profile = "переглянути статистику акаунту"
...
# - Опис змінних
v_edit_greeting_text = "Введи текст вітального повідомлення:"
v_edit_greeting_cooldown = "Введи кулдаун для привітання (в днях, наприклад, 0.5 для 12 годин):"
...
# Логи (залишаємо переважно як є, оскільки це для розробника, а не користувача)
...
log_tg_notification_error = "Сталася помилка під час надсилання сповіщення в чат $YELLOW{}$RESET."
...
log_greeting_cooldown_changed = "$MAGENTA@{} (ID: {})$RESET змінив кулдаун вітального повідомлення на $YELLOW{}$RESET дн."
...
log_gf_new_goods = "$MAGENTA@{} (ID: {})$RESET додав $CYAN{}$RESET товар(-у, -ів) у файл $YELLOWstorage/products/{}$RESET."
...
log_tmplt_added = "$MAGENTA@{} (ID: {})$RESET додав заготівлю відповіді $YELLOW\"{}\"$RESET."
log_tmplt_deleted = "$MAGENTA@{} (ID: {})$RESET видалив заготівлю відповіді $YELLOW\"{}\"$RESET."
...
log_pl_delete_handler_err = "Сталася помилка під час виконання хендлера видалення плагіна $YELLOW\"{}\"$RESET."

# Логи хендлерів
log_new_msg = "$MAGENTA┌──$RESET Нове повідомлення в листуванні з користувачем $YELLOW{} (CID: {}):"
log_sending_greetings = "Користувач $YELLOW{} (CID: {})$RESET написав вперше! Надсилаю вітальне повідомлення..."
log_new_cmd = "Отримано команду $YELLOW{}$RESET в чаті з користувачем $YELLOW{} (CID: {})$RESET."
...
ntfc_new_order_ad_disabled_for_lot = "ℹ️ Товар не буде видано, оскільки авто-видача вимкнена для даного лота."
ntfc_new_order_user_blocked = "ℹ️ Товар не буде видано, оскільки користувач знаходиться в ЧС і увімкнено блокування авто-видачі."
...
crd_acc_get_unexpected_err = "Сталася непередбачена помилка при отриманні даних акаунту."
...
crd_lots_raised = "Усі лоти категорії $CYAN\"{}\"$RESET піднято!"
...
crd_handler_err = "Сталася помилка під час виконання хендлера."
crd_tg_au_err = "Не вдалося змінити повідомлення з інформацією про користувача: {}. Спробую без посилання."

# Нові ключі, додані в процесі редизайну:

# Для utils.py -> generate_profile_text
# profile_header_title = "Статистика акаунту" # Вже є як cmd_profile
# acc_balance_available = "доступно" # Вже є вище

# Для utils.py -> generate_lot_info_text
# lot_info_header = "Інформація про лот" # Вже є вище
# text_not_set = "(текст не задано)" # Вже є вище
# gf_infinity = "нескінченно" # Вже є вище
# gf_count_error = "помилка підрахунку" # Вже є вище
# gf_file_not_found_short = "не знайдено" # Вже є вище
# gf_not_linked = "не прив'язано" # Вже є вище
# gf_file_created_now = "створено" # Вже є вище
# gf_file_creation_error_short = "помилка створення" # Вже є вище

# Для auto_delivery_cp.py
# never_updated = "ніколи" # Вже є вище
# ad_default_response_text_new_lot = "Дякуємо за покупку, $username!\nОсь твій товар:\n$product" # Вже є вище
# no_lots_using_file = "Жоден лот не використовує цей файл." # Вже є вище
# gf_no_products_to_add = "Не було введено товарів для додавання." # Вже є вище
# gl_refresh_and_try_again = "Оновіть список і спробуйте знову." # Вже є вище
# gf_deleted_successfully = "Файл {file_name} успішно видалено." # Вже є вище
# gf_already_deleted = "Файл {file_name} вже було видалено." # Вже є вище

# Для auto_response_cp.py
# ar_no_valid_commands_entered = "Не введено жодної коректної команди." # Вже є вище
# ar_default_response_text = "Відповідь для цієї команди ще не налаштована. Відредагуйте її." # Вже є вище
# ar_default_notification_text = "Користувач $username ввів команду: $message_text" # Вже є вище
# ar_command_deleted_successfully = "Команду/сет '{command_name}' успішно видалено." # Вже є вище

# Для file_uploader.py
file_err_not_detected = "❌ Файл не знайдено в повідомленні."
file_err_must_be_text = "❌ Файл повинен бути текстовим (наприклад, .txt, .cfg, .py, .json, .ini, .log)."
...
file_err_download_failed = "❌ Сталася помилка при завантаженні файлу на сервер бота."
...
plugin_uploaded_success = "✅ Плагін <code>{filename}</code> успішно завантажено.\n\n⚠️ Щоб плагін запрацював, <b><u>перезавантажте FPCortex</u></b> командою /restart!"
...
image_upload_error_generic = "⚠️ Не вдалося вивантажити зображення на FunPay. Деталі дивіться в лог-файлі (<code>logs/log.log</code>)."
...
products_file_provide_prompt = "📎 Надішліть мені файл з товарами (у форматі .txt):"
products_file_upload_success = "✅ Файл з товарами <code>{filepath}</code> успішно завантажено.\nКількість товарів у файлі: <code>{count}</code>."
products_file_count_error = "⚠️ Сталася помилка при підрахунку товарів у завантаженому файлі. Файл завантажено, але перевірте його вміст."
...
# Для plugins_cp.py
# pl_status_active = "Статус: Активний 🚀" # Вже є вище
# pl_status_inactive = "Статус: Неактивний 💤" # Вже є вище
# pl_no_commands = "У цього плагіна немає зареєстрованих команд." # Вже є вище
# pl_delete_handler_failed = "..." # Вже є вище
# pl_deleted_successfully = "..." # Вже є вище
# pl_file_delete_error = "..." # Вже є вище

# Для proxy_cp.py
# proxy_status_enabled = "Увімкнено" # Вже є вище
# proxy_status_disabled = "Вимкнено" # Вже є вище
# proxy_check_status_enabled = "Увімкнена" # Вже є вище
# proxy_check_status_disabled = "Вимкнена" # Вже є вище
# proxy_not_used_currently = "не використовується" # Вже є вище
# proxy_not_selected = "не обрано" # Вже є вище
# proxy_check_interval_info = "Інтервал авто-перевірки: {interval} хв." # Вже є вище
# proxy_global_status_header = "Загальний статус проксі-модуля" # Вже є вище
# proxy_module_status_label = "Стан модуля:" # Вже є вище
# proxy_health_check_label = "Авто-перевірка проксі:" # Вже є вище
# proxy_current_in_use_label = "Поточний проксі в роботі:" # Вже є вище
# proxy_select_error_not_found = "..." # Вже є вище
# proxy_select_error_invalid_format = "..." # Вже є вище
# proxy_selected_and_applied = "..." # Вже є вище
# proxy_selected_not_applied = "..." # Вже є вище
# proxy_deleted_successfully = "..." # Вже є вище
# proxy_delete_error_not_found = "..." # Вже є вище

# Для templates_cp.py
# tmplt_editing_header = "📝 Редагування шаблону" # Вже є вище
# tmplt_err_empty_text = "..." # Вже є вище
# tmplt_deleted_successfully = "..." # Вже є вище

# END OF FILE FunPayCortex/locales/uk.py