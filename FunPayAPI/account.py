...
class Account:
    ...
    def upload_image(self, image: str | IO[bytes], type_: Literal["chat", "offer"] = "chat") -> int:
        ...
        fields = {
            'file': ("Отправлено_с_помощью_бота_FunPay_Cortex.png", img, "image/png"),
            'file_id': "0"
        }
    ...
    def __parse_messages(self, json_messages: dict, chat_id: int | str,
        ...
        for i in json_messages:
            ...
            if self.chat_id_private(chat_id) and (image_tag := parser.find("a", {"class": "chat-img-link"})):
                ...
                image_link = image_tag.get("href")
                message_text = None
                # "Отправлено_с_помощью_бота_FunPay_Cortex.png", "funpay_cortex_image.png"
                if isinstance(image_name, str) and "funpay_cortex" in image_name.lower():
                    by_bot = True
                elif image_name == "funpay_vertex_image.png": # TODO: Consider renaming this too if needed?
                    by_vertex = True
                ...
                if message_text.startswith(self.__bot_character) or \
                    ...
                    by_bot = True
                # todo придумать, как отсеять юзеров со старыми версиями кардинала/кортекса (подождать обнову фп?)
